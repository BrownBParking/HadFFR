...
if __name__ == "__main__":
	...
	if args.fits:
		for side in ["tag", "probe"]:
			chain = ROOT.TChain("Bcands_{}match_Bs2PhiJpsi2KKMuMu_probefilter".format(side))
			...
			for cut_name in cuts:
				...
				plot_mc(chain, cut=cut_str, tag="Bs_{}_{}".format(side, cut_name))

				ws, fit_result = fit_mc(chain, cut=cut_str)
				ws.Print()
				fit_result.Print()
				ws_file = ROOT.TFile("fitws_mc_Bs_{}_{}.root".format(side, cut_name), "RECREATE")
				ws.Write()
				fit_result.Write()
				ws_file.Close()
	...
	if args.plots:
		for side in ["tag", "probe"]:
			for cut_name in cuts:
				ws_file = ROOT.TFile("fitws_mc_Bs_{}_{}.root".format(side, cut_name), "READ")
				#ws_file.ls()
				ws = ws_file.Get("ws")
